...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!list_empty(&exp_data.list)) {
		list_for_each_entry(exp_fhandler, &exp_data.list, link)
			...
			{
				tp_log_info("%s,line = %d,exp_fhandler->exp_fn->reset = %pf\n",
					__func__,__LINE__,exp_fhandler->exp_fn->reset);
	...
	if (!list_empty(&exp_data.list)) {
		list_for_each_entry_safe(exp_fhandler,
			...
			if ((exp_fhandler->exp_fn->init != NULL) &&
				...
				tp_log_info("%s,line = %d,exp_fhandler->exp_fn->init = %pf,retval=%d\n",
					__func__,__LINE__,exp_fhandler->exp_fn->init,retval);
				if ((retval < 0) && (!rmi4_data->flash_prog_mode)) {	
					tp_log_info("%s: line(%d)\n", __func__,__LINE__);
					/*if upgrade fail the flash_prog_mode is set 1,else set 0*/
