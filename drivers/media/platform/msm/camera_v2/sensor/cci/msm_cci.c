...
#define HW_CCI_TIMEOUT 100
#else
#define HW_CCI_TIMEOUT CCI_TIMEOUT
#endif
	...
	enum cci_i2c_master_t master)
{
	...
	if (rc < 0) {
		hw_camera_log_error("%s:%d wait failed  rc = %d\n", __func__, __LINE__, rc);
	} else if (rc == 0) {
		...
		rc = wait_for_completion_interruptible_timeout(
			...
			CCI_TIMEOUT);
		if (rc <= 0)
			hw_camera_log_error("%s:%d wait failed %d\n", __func__, __LINE__,
				rc);
	}
	return;
}
	...
	enum cci_i2c_queue_t queue)
{
	...
	if ((read_val + len + 1) > cci_dev->
		...
		if (rc <= 0) {
			hw_camera_log_error("%s: wait_for_completion_interruptible_timeout %d rc = %d\n", __func__, __LINE__, rc);
			if (rc == 0)
				rc = -ETIMEDOUT;
	...
	struct msm_camera_cci_ctrl *c_ctrl)
{
	...
	rc = wait_for_completion_interruptible_timeout(&cci_dev->
		...
		if(-ERESTARTSYS == rc)
		{
			hw_camera_log_error("%s %d : rc = -ERESTARTSYS \n", __func__,__LINE__);
			continue;
		}
		else
		{
	...
	}while(cost_time < HW_CCI_TIMEOUT);
	...
	if (read_words != exp_words) {
		hw_camera_log_error("%s:%d read_words = %d, exp words = %d\n", __func__,
	...
	struct msm_camera_cci_ctrl *c_ctrl)
{
	...
	rc = wait_for_completion_interruptible_timeout(&cci_dev->
		...
		if(-ERESTARTSYS == rc)
		{
			hw_camera_log_error("%s %d : rc = -ERESTARTSYS cost_time = %u\n", __func__,__LINE__,cost_time);
			continue;
		}
		else
		{
...
{
	int32_t rc = 0;
	uint32_t start_time = 0, start_time2 = 0;
	uint32_t cost_time = 0, cost_time2 = 0;
	struct cci_device *cci_dev;
	enum cci_i2c_master_t master;
	cci_dev = v4l2_get_subdevdata(sd);
	...
	rc = wait_for_completion_interruptible_timeout(
		...
		CCI_TIMEOUT);
		cost_time2 = (jiffies-start_time2)*1000/HZ;
	...
	}while(cost_time2 < HW_CCI_TIMEOUT);
