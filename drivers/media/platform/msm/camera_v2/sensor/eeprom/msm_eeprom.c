				       ...
				       struct msm_eeprom_cfg_data *cdata)
{
	...
	/*fix bug:daemon thread restart, camera conn't used, must be restart phone*/
			     ...
			     void __user *argp)
{
	...
	case CFG_EEPROM_GET_CAL_DATA:
		...
		hw_camera_log_info("%s e_ctrl->cal_data.num_data %d\n", __func__, e_ctrl->cal_data.num_data);
				       ...
				       struct msm_eeprom_memory_block_t *data)
{
	...
	for (i = 0; i < data->num_map; i++) {
		...
		rc = of_property_read_u32_array(of, property,
				(uint32_t *) &map[i].slave_addr, count);
		if (rc < 0) {
		...
		hw_camera_log_info("%s map[%d].mem.valid_size %d\n", __func__, i, map[i].mem.valid_size);
	}
...
static int msm_eeprom_platform_probe(struct platform_device *pdev)
{
	...
	rc = msm_eeprom_get_dt_data(e_ctrl);
	if (rc){
		hw_camera_log_error("msm_eeprom_get_dt_data failed rc = %d\n", rc);
		goto board_free;
	}

	rc = msm_eeprom_parse_memory_map(of_node, &e_ctrl->cal_data);
	if (rc < 0){
		hw_camera_log_error("msm_eeprom_parse_memory_map failed rc = %d\n", rc);
		goto board_free;
	}
	...
	for (j = 0; j < e_ctrl->cal_data.num_data; j++)
		hw_camera_log_debug("memory_data[%d] = 0x%X\n", j,
