	...
	struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	ret = of_property_read_u32(of_node, "qcom,sensor-ae-meter-type",
		&sensordata->sensor_info->ae_meter_type);
	if(ret<0) {
		printk("CAM_DBG :no ae_meter_type\n");
		sensordata->sensor_info->ae_meter_type = 0;
	}
...
/*add check moudle id info*/
	/*move s_ctrl->func_tbl->sensor_match_module() (check module id function)to*/
	/*function msm_sensor_platform_probe()*/
	if (rc < 0)
...
int msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	...
	case CFG_GET_SENSOR_INFO:
		...
		cdata->cfg.sensor_info.ae_meter_type =
			s_ctrl->sensordata->sensor_info->ae_meter_type;
	...
	case CFG_SET_SLAVE_INFO: {
		...
		if (!sensor_slave_info.power_setting_array.power_setting||
			0 == size) {
			pr_err("%s: Missing dedicated power up sequence\n",
				__func__);
			break;
		}
...
int32_t msm_sensor_platform_probe(struct platform_device *pdev, void *data)
{
    ...
    if(s_ctrl->func_tbl->sensor_match_module)
    {
        ...
        if(rc < 0)
        {
            pr_err("%s %s sensor_match_module failed\n", __func__,
			s_ctrl->sensordata->sensor_name);
		    kfree(s_ctrl->sensordata->power_info.clk_info);
		    kfree(cci_client);
            return rc;
	...
	switch(s_ctrl->sensordata->sensor_info->position)
	{
		case BACK_CAMERA_B:
			...
			printk("%s: %s set_hw_dev_flag_main success!! \n",__func__,s_ctrl->sensordata->sensor_name);
			break;
		case FRONT_CAMERA_B:
			set_hw_dev_flag(DEV_I2C_CAMERA_SLAVE);
			printk("%s: %s set_hw_dev_flag_slave success!! \n",__func__,s_ctrl->sensordata->sensor_name);
			break;
		default:
