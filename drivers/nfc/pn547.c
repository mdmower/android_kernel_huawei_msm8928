			 ...
			 struct pn547_i2c_platform_data *pdata)
{
	...
	pdata->irq_gpio =  of_get_named_gpio_flags(np, "huawei,nfc_int", 0,NULL);
	if (pdata->irq_gpio < 0) {
		pr_err( "failed to get \"huawei,nfc_int\"\n");
		goto err;
	}

	pdata->fwdl_en_gpio = of_get_named_gpio_flags(np, "huawei,nfc_firm", 0,NULL);
	if (pdata->fwdl_en_gpio< 0) {
		pr_err( "failed to get \"huawei,nfc_firm\"\n");
		goto err;
	}
	
	pdata->ven_gpio = of_get_named_gpio_flags(np, "huawei,nfc_ven", 0,NULL);
	if (pdata->ven_gpio < 0) {
		pr_err( "failed to get \"huawei,nfc_ven\"\n");
		goto err;
	}
	 
	pdata->clk_req_gpio = of_get_named_gpio(np, "huawei,nfc_clk", 0);
				...
				struct pn547_i2c_platform_data *pdata)
{
	...
	if(rc < 0) {
		printk("failed to gpio_tlmm_config irq_gpio\n");
	}
	...
	if(ret)
		goto err_fwdl_en;
	//gpio_direction_output(pdata->fwdl_en_gpio, 1);
	gpio_config = GPIO_CFG(pdata->fwdl_en_gpio, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA);
	rc = gpio_tlmm_config(gpio_config, GPIO_CFG_ENABLE);
	if(rc < 0) {
		printk("failed to gpio_tlmm_config fwdl_en_gpio\n");
	}
	gpio_direction_output(pdata->fwdl_en_gpio,0);

	ret=gpio_request(pdata->ven_gpio,"nfc_ven");
	if(ret)
		goto err_ven;
	//gpio_direction_output(pdata->ven_gpio, 0);

	ret=gpio_request(pdata->clk_req_gpio,"nfc_clk_req");
	if(ret)
		goto err_clk_req;
	//gpio_direction_output(pdata->clk_req_gpio);	
...
static int pn547_remove(struct i2c_client *client)
{
	...
	if (NULL == pn547_dev){
		dev_err(&client->dev, "%s ++ i2c_get_clientdata error\n", __func__);
		return -1;
	}
	free_irq(client->irq, pn547_dev);
	misc_deregister(&pn547_dev->pn547_device);
	if (&pn547_dev->read_mutex)
		mutex_destroy(&pn547_dev->read_mutex);
