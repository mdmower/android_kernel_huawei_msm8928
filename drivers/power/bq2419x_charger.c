...
static void bq2419x_usb_otg_work(struct work_struct *work)
{
    ...
    /*read consumed battery current*/
    consumed_current = bq27510_battery_current(g_battery_measure_by_bq27510_device);
    if(MAX_ALLOWED_CURRENT >= consumed_current){
        over_current_count++;
    }else{
        over_current_count = 0;
    }
    if(OVER_CURRENT_COUNT == over_current_count){
        di->chrg_config = DIS_CHARGER;
        bq2419x_config_power_on_reg(di);
        over_current_count = 0;
    }
    ...
    schedule_delayed_work(&di->bq2419x_usb_otg_work,
                 msecs_to_jiffies(BQ2419x_OTG_TIMEOUT));
}
                           ...
                           const struct i2c_device_id *id)
{
    ...
    di->hot_design_current = di->max_cin_currentmA;
	/* runnig test flag, used for forcing IC discharge */
