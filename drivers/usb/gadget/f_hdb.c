...
#define HDB_BULK_BUFFER_SIZE           4096
...
requeue_req:
    /* queue a request */
    req = dev->rx_req;
    req->length = HDB_BULK_BUFFER_SIZE;
    dev->rx_done = 0;
...
static int hdb_open(struct inode *ip, struct file *fp)
{
    ...
    atomic_set(&_hdb_dev->error, 0);

    if (_hdb_dev->close_notified) {
        _hdb_dev->close_notified = false;
        hdb_ready_callback();
    }

    _hdb_dev->notify_close = true;
    return 0;
}
...
static int hdb_release(struct inode *ip, struct file *fp)
{
    ...
    /*
     * HDB daemon closes the device file after I/O error.  The
     ...
     * commands like "hdb root" and "hdb usb".
     */
    if (_hdb_dev->notify_close) {
        hdb_closed_callback();
        _hdb_dev->close_notified = true;
    }
...
static int hdb_bind_config(struct usb_configuration *c)
{
    ...
    pr_debug("hdb_bind_config\n");

    dev->cdev = c->cdev;
    dev->function.name = "hdb";
    dev->function.descriptors = fs_hdb_descs;
...
static int hdb_setup(void)
{
    ...
    /* config is disabled by default if hdb is present. */
