			  ...
			  unsigned long event_state)
{
	...
	if (event_state & (1 << MBHC_EVENT_PA_HPHL)) {
		...
		goto exit;
         ...
         /*
	 * and just use for MATE2 because MATE2 supporting TTY device
	 * so add a function to detect whether device is MATE2.
	 * if Mate2 Use that patch, other devices not use it, otherwise will cause a headphone
	 * sometimes detected as headset
	 */
#ifdef CONFIG_HUAWEI_KERNEL
